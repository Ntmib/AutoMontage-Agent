...
- Windows cleanup повторяет только временные `EPERM`/`EBUSY`/`ENOTEMPTY` при удалении
  identity-проверенного пустого tombstone; чужие и недоказанные остатки сохраняются, а
  завершившийся `rmdir`, сообщивший ошибку, согласуется без ложного orphan.
