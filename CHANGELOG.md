...
- Windows: `writeManifestAtomic` открывал временный манифест порции как `'r'` и вызывал
  `fsyncSync` на дескрипторе только для чтения. Windows отклоняет `FlushFileBuffers` на таком
  хэндле (`EPERM: operation not permitted, fsync`), из-за чего порционный рендер падал на первой
  же порции и `automontage` не собирал ни одного ролика длиннее 540 кадров. Дескриптор
  открывается как `'r+'`.
- `--scenario`: поля `audio` и `theme` монтажного листа не читались в `scripts/build.js`.
  Схема `schema/scenario.schema.json` их разрешает, валидация проходила, но до `props` они не
  доходили - фоновая музыка молча пропадала, а тема всегда бралась из флага `--theme`
  (по умолчанию `craft`). Флаг `--theme` по-прежнему важнее листа.

### Добавлено

- Тема поддерживает режим `plain` - текст поверх кадра без подложки, рамки и тени-плашки,
  с настраиваемой тенью текста (`textShadow`) и кеглями (`subtitleSize`, `subSize`,
  `ctaHeadSize`, `ctaBtnSize`). По умолчанию выключен, вид существующих тем не меняется.
- Тест `tests/chunk-cache-manifest.test.js`: проверяет запись манифеста на реальной ФС и на
  файловой системе, отклоняющей `fsync` дескриптора только для чтения (как Windows).
