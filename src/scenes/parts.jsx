...
export const SceneBg = ({ chrome = true }) => {
  ...
  return (
    <>
      ...
      {chrome && k.D.orbit && <div style={{ position: 'absolute', right: -140, top: 200, width: 900, height: 900, borderRadius: '50%', border: `2px dotted ${k.orange}55` }} />}
      {chrome && k.D.stars && <>{star({ right: s.right + 20, top: 360, fontSize: 54 })}{star({ left: s.left + 20, bottom: s.bottom + 120, fontSize: 36 })}</>}
      {chrome && k.D.plus && <>{plus({ right: s.right + 40, top: 720, fontSize: 46 })}{plus({ left: s.left, top: 980, fontSize: 40 })}</>}
