...
Запускай только одну сборку на checkout. Исходник для Remotion теперь получает отдельный
временный lease в `public/.automontage/`, поэтому не подменяется другим build, но `tmp/` и
legacy-пути всё ещё общие. Для параллельного монтажа нужны отдельные clone/worktree.
