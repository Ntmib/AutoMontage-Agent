...
не даёт двум Review-серверам занять одну ревизию. При `409` браузер загружает свежий disk state,
но не повторяет накопленные команды автоматически. Approved-ревизии остаются неизменными.
...
только его путь. Файл удаляется при закрытии сервера или через 10 минут. Локально прочитай URL
из этого файла и передай его браузеру, не копируя в чат или журнал. Если ffmpeg не сумел
