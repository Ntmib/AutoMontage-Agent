...
Пути из `project.json` не считаются доверенными: движок принимает только канонические
относительные пути внутри workspace и отклоняет выход через symlink. Эта проверка рассчитана
на manifest и symlink, существующие в момент проверки; не давай недоверенным процессам запись
в папку проекта.
