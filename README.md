...
b-roll на файл из server allowlist. Перенос меняет `end` левой и `start` правой сцены; поздние
сцены не сдвигаются. Undo/redo живут только в памяти браузера до явного Save. Save не
перезаписывает выбранный draft или approved brief: он атомарно создаёт новую пару
`vNN-draft.lesson.md` + `.json` и одну запись manifest. Approved-ревизии остаются неизменными.
...
Ссылка с token даёт доступ к локальному предпросмотру, поэтому её не публикуют. Если ffmpeg не
сумел построить waveform, Review всё равно открывается с обычной временной шкалой.
