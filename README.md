...
1. Нажми **«Добавить медиа»** и выбери один файл. Дождись сообщения об успехе. Во время
   обработки запрос может оставаться открытым несколько минут: сервер делает безопасную
   рабочую копию внутри именно той project-папки, которая передана в `--project-dir`.
...
4. Для видео сначала прокрути маленький плеер к нужному моменту и нажми
   **«Начать с текущего места»**. Review привяжет старт к ближайшему кадру. Затем выбери
   положение и звук:
...
`старт + длительность сцены ≤ длительность видео` с покадровым округлением. Импорт идёт по
одному файлу, требует свободного места примерно `4 × размер файла + 512 MiB` и может заметно
нагрузить CPU: мастер нормализуется, а для видео отдельно создаётся браузерный WebM-прокси.
...
В первой версии импортированные активы неизменяемы: нет удаления или перезаписи файла тем же id.
...
Межпроцессный exclusive reservation не даёт двум Review-серверам занять одну ревизию. При
