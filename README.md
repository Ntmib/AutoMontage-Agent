...
Approval публикует approved JSON только после согласованных Markdown и `project.json`; при
ошибке записи промежуточные файлы очищаются, а прежний draft остаётся текущим.
...
`--edit` явно включает только две правки: перенос общей границы двух соседних сцен и замену
b-roll на изображение AVIF/GIF/JPEG/PNG/WebP из server allowlist. Аудио и видео остаются видны
в общей media-дорожке, но сервер не разрешит подать их в текущий image-only b-roll renderer.
Перетаскивание границы притягивается к ближайшему слову или кадру; стрелки двигают строго по
кадрам, поэтому не застревают на слове. Timing audit отдельно помечает frame- и word-подсказки.
Перенос меняет `end` левой и `start` правой сцены; поздние сцены не сдвигаются. Undo/redo живут
только в памяти браузера до явного Save. Save не
перезаписывает выбранный draft или approved brief: он атомарно создаёт новую пару
`vNN-draft.lesson.md` + `.json` и одну запись manifest. Межпроцессный exclusive reservation
не даёт двум Review-серверам занять одну ревизию. Markdown и JSON становятся видимыми раньше
ссылающейся на них manifest entry, поэтому читатель не увидит текущую ревизию без JSON. При
`409` браузер немедленно очищает активные undo/redo-команды и блокирует timeline, b-roll и Save,
а затем загружает свежий disk state. Если этот запрос не удался, quarantine сохраняется, а
кнопка продолжения остаётся недоступной. Устаревшие правки не применяются: только после успешной
загрузки новой версии явная кнопка «Отбросить устаревшие правки и продолжить» снимает блокировку,
и новая команда
проверяется уже от свежей базы. Approved-ревизии остаются неизменными.
