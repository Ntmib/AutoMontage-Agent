...
изображение. Это отдельная платная операция и она никогда не выводится из самой просьбы
«смонтируй ролик». Существующий `scripts/gen-brief.js` сохраняется как explicit
legacy/developer provider path для совместимости кода, но не является стандартным шагом навыка
или публичного beginner workflow.
